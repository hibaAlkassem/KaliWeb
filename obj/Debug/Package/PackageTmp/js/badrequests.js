...
$(document).ready(function () {
    ...
    var d = $('#devices').DataTable({
        bFilter: false,
        "width": "10%",
        "sAjaxSource": config_apiserver + 'api/importData/' + myval + '/table/badrequests',
        "sAjaxDataProp": "",
        "bAutoWidth": false,
        "bLengthChange": false,
    ...
    $('#deviceIdDropdown').change(function () {
        reloadTable($('#deviceIdDropdown').val());
    });
    ...
    function reloadTable(deviceId) {
        ...
        if (deviceId == "-1")
            var url = config_apiserver + 'api/importData/' + myval + '/table';
        else
            var url = config_apiserver + "api/importdata/" + myval + '/getLoggedData?deviceId=' + deviceId;
    ...
    $.ajax({
        ...
        success: function (response) {
            for (var d in response)
                $('#deviceIdDropdown').append($("<option />").val(response[d].DeviceId).text(response[d].DeviceHardwareId));
        },
        error: function () {
