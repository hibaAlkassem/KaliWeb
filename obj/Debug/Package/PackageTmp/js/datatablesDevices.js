...
$(document).ready(function() {
	...
	$.ajax({
	    type: "GET",
	    url: config_apiserver + "/api/devices",
	    data: "",
	    contentType: "application/json; charset=utf-8",
	    dataType: "json",
	    async: true,
	    success: function (response) {
	        devicesData = processRequest(response);
...
function processRequest(data) {
    ...
    for (var p in data) {
        ...
        var temp = ["<i class=\"fa fa-plus-circle\"></i>", "<strong>" + data[p].DeviceTypeName + "</strong>" + getOnlineImage(data[p].DeviceStatus), data[p].DeviceUserName, getBatteryImage(data[p].PowerBattery), data[p].DeviceKey];
...
function getActivityDiv(val, id) {
    var deviceacts = getDeviceActivities(id);
    if (deviceacts.Count < 1)
        return "";
    var actval = "";
    for (var act in deviceacts) {
        actval = deviceacts[act].PowerBattery + ",";
    }
    actval = actval.substring(0, actval.length - 1);
    return "<div class='customer-sparkline' id='sparkgraph" + id + "' data-sparkline-color='#0aa699' data-sparkline-type='bar'><!-- " + actval + " --></div> ";
}

function getOnlineImage(status) {
    if (status.toUpperCase() == "online".toUpperCase())
        return "<img src='images/online1.png' style='margin-left:15px' class='tip' data-toggle='tooltip' title='' data-original-title='online'/>";
    else
        return "<img src='images/offline.png' style='margin-left:15px' class='tip' data-toggle='tooltip' title='' data-original-title='offline'/>";
}

function getBatteryImage(battery) {
    //return "<img src='assets/img/logo-b.png' class='btn btn-white tip' data-toggle='tooltip' title='' data-original-title='Up here!' />";
    if (battery == 1)
        return "<img src='images/battery_1.png' class='tip' data-toggle='tooltip' title='' data-original-title='90% remaining'/>";
    else
        return "<img src='images/battery_2.png' class='tip' data-toggle='tooltip' title='' data-original-title='20% remaining'/>";
}

function getDeviceActivities(deviceId) {
    $.ajax({
        type: "GET",
        url: config_apiserver + "api/devices/" + deviceId + "/activities",
        data: "",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        async: true,
...
function fillDevicesTable(handlerData) {

    for (var data in handlerData.aaData)
        $('#datatable_devices').dataTable().fnAddData([
