...
$.extend( true, $.fn.dataTable.defaults, {
	...
	"oLanguage": {
	    "sLengthMenu": "_MENU_",
	    "sEmptyTable": "Loading Activities..."
...
$(document).ready(function () {
    ...
    oTable = $('#datatable_activities').dataTable({
        ...
        "fnRowCallback": function (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
               var s = '<span class="hidden"> ' + aData[1] + ' </span> <abbr class="timeago tip" data-toggle="tooltip" title="' + aData[1] + ' UTC">' + getDateString(aData[1]) + '</abbr>';
            $('td:eq(1)', nRow).html(s);
        },
 ...
 processActivitiesFile();
    ...
    //});
		...
		$.ajax({
		    type: "GET",
		    url: config_apiserver + "/api/activities/all/" + myval,
		    data: "",
		    ...
		    success: function (response) {
		        ...
		        activitiesHandlerData = processRequest(response);
		        fillActivitiesTable(activitiesHandlerData);

		    },
		    error: function () {

		        console.log("error at activities handler");
		        return;
		    }
		});
...
function startListening() {
    ...
    actHub.client.getData = function (e) {
        console.log(e);
        if (paused == 0) {
    ...
    $.connection.hub.start().done(function () {
        ...
        console.log(lastid);
        //activitiesHub.server.getActivites(lastid);
...
function processRequest(data) {
    ...
    for (var p in data) {
        //var time = '<div class=" timeago tip" datatoggle="tooltip" data-original-title="sada"><abbr data-original-title="' + getDate(data[p].TimeStamp) + '"  class="timeago tip" data-toggle="tooltip" title="">' + jQuery.timeago(data[p].TimeStamp) + '</abbr></div>';
        var temp = ["<i class='fa fa-plus-circle'></i>", data[p].TimeStamp, "/devices/" + data[p].DeviceId, data[p]
            .ActivitySummary, data[p].ActivityId, data[p].SensorId
...
function fillActivitiesTable(handlerData) {

    for (var data in handlerData.aaData)
        $('#datatable_activities')
        .dataTable()
        .fnAddData([

            handlerData.aaData[data][0], handlerData.aaData[data][1], handlerData.aaData[data][2], handlerData.aaData[
                data][3],
            handlerData.aaData[data][4], handlerData.aaData[data][5]
