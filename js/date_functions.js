...
function getTimeagoDate(taDate) {

    var ymd = taDate.getUTCFullYear() + "-" + (taDate.getUTCMonth() + 1).toString() + "-" + taDate.getDate() + " ";

    var dat = new Date(taDate);
   
    if ($.browser.mozilla) {
        dat.setUTCHours(taDate.getHours(), taDate.getMinutes(), taDate.getSeconds(), taDate.getMilliseconds());
    }
    ...
    var ret = ymd + hour + ':' + min + ':' + sec;
   // console.log("timeago", taDate.toString());
    return ret;
